    ...
    return (
        (remove)&&(
            <div className='zzimcomp'>
                <div className='imgcontainer'> <Link  to={`/spaceDetail/${record.space.sseq}`}>{(record.space.spaceimage&&record.space.spaceimage.length>0)&&(<img src={`https://final-kimminju.s3.ap-northeast-2.amazonaws.com/space_images/${record.space.spaceimage[0].realName}`}/>)}</Link></div>
