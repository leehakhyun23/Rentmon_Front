  ...
  function SpaceDetail() {
    ...
    return (
      <div className='spaceContainer innerContainer'>
        <div>
          {/* spaceMenu Part */}
          <div className={`SpaceMenu-container ${inquiryopen || reviewopen || reportopen ? 'hidden' : ''}`}>            <div className="SpaceMenu-item" onClick={() => scrollToSection(spaceInfoRef)}>공간 정보</div>
            ...
            <div className="spaceContentBlock">
              ...
              <div className="spaceContent">
                {tagList ? tagList.map((tag, idx) => (
                  <span key={idx} style={{ marginRight: '10px' }}>#{tag.word}</span>
                )) : null}
              </div>
            </div>
          </div>
